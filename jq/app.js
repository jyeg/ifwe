...
$(document).ready(function () {
	...
	$('#newGameForm').submit(function (e) {
		...
		// TODO: clear form, validate for edge cases, same number, write tests
		...
		scores.push(tempGame);

		var inputs = this.getElementsByTagName('input');
		for(var i = 0; i < inputs.length; i++){
				if(inputs[i].getAttribute("type") === "text" || inputs[i].getAttribute("type") === "number"){
					inputs[i].value = "";
				}
		}
		// add new score
	...
	$('.person').bind('click', 'li', function(e){
		activeUserName = $(this).val();
...
// event on leaderboard
// populate the gamesPlayed
// with hidden input fields that when clicked become editable.
// and on save or enter, update scores array.
